...
Every number in `components/basket-sheet.tsx` is the server's. `hooks/use-basket-price.ts` posts the lines to `priceUrl` with Inertia's `useHttp` whenever the sheet is open and the basket changes (`App\Actions\Menus\PriceBasket`, see `.ai/rules/actions-menus.md`). Two traps in that hook:
