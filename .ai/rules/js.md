...
**GST is named once in the totals, and every figure that names it adds up.** The project owner's instruction after reading a basket that stated the same tax three times: on the line, as a total, and halved into CGST and UTGST.

What the basket draws now:
- **Each line** carries its own rate and amount ("Incl. GST 18% · ₹45.62"), because one basket can hold a 5% item beside an 18% one and a single figure at the foot would hide that.
- **Each charge** carries its own, in the same words. It did not, and that was the actual defect: a line reading ₹45.62 sat above a foot reading ₹53.24 with the fee's ₹7.62 shown nowhere, so the two numbers looked unrelated. Charges are taxed with the supply (`.ai/rules/actions-menus.md`), so they say so.
- **One figure in the totals.** Where the prices include GST it is a footnote under the total — "Includes GST of ₹53.24" — because it is not being added; where they exclude it, it is one `GST` row inside the sum, because it is.

**The CGST/UTGST split is deliberately not drawn in the guest app.** A guest reads a basket, not a tax invoice, and this sheet says so in its own subtitle. The split is still worked out, still stored on the order, and still shown in the panel's order infolist, which is the record a tax invoice is raised from (`.ai/rules/enums.md`). `priced.taxParts` and `priced.isUnionTerritory` are still on the wire and still read per line for the rate — nothing derives a split in the browser (halving a total does not reliably add back up; see `.ai/rules/actions-menus.md`). There is no IGST anywhere. A complimentary line shows no GST line at all, and a tenant charging none shows nothing.
