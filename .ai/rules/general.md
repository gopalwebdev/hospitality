...
- **Format in the browser, decide in PHP.** Standing instruction from the project owner, and it is about the server's CPU as much as about the reader: money, dates, times and numbers are formatted client-side, so nothing builds a string per row. Money is stored and sent as an integer in the currency's minor unit (₹249.50 is `24950`) and a time of day as the stored `HH:MM`; `resources/js/lib/money.ts` and `lib/time.ts` turn them into something to read, in the reader's own language. The **one** exception is a Filament panel, which is server rendered and has no client to format in — see `.ai/rules/js.md`. Anything that is security-relevant or a decision — authorisation, validation, what a guest may see, what a tile points at — stays in PHP.
...
- **PHP 8.5 and Node 24 only.** `composer.json` requires `~8.5.0` and pins `config.platform.php` to `8.5.0`, so resolution never assumes a newer runtime; a `check-php` script on `pre-install-cmd` and `pre-update-cmd` aborts Composer on anything outside 8.5, because a platform override otherwise lets `install` succeed on the wrong version and fail later at runtime. Node is `~24` in `package.json` `engines` with `engine-strict=true` in `.npmrc`, and `.nvmrc` is the one place the version is written — CI's setup action reads it. Locally, `herd use 8.5` sets the CLI version; Herd's `php` shim is global, so a site isolated to 8.5 does not make the CLI 8.5. Moving to a newer runtime means changing these pins deliberately.
...
Say **item** (`MenuItem`, "Items"), **add-on group** for `menu_add_on_groups` and **option** for `menu_add_on_options` ("Add-on groups", never modifiers), **diet** for the veg / vegan / egg / non-veg marks (`App\Enums\Diet`, `menu_items.diets`, drawn by `ItemMark`), and **service request** for `menu_items.is_service_request`. A price of 0 is **complimentary**. `grep -rniE "food|dish"` over app, database, resources, lang, tests and `.ai/rules` should find nothing but this rule, which has to name the words it forbids.

## One date format and one clock, everywhere: "Aug 6, 2025", 12-hour, no seconds
Standing instruction from the project owner. A date reads "Aug 6, 2025"; a date and time read "Aug 6, 2025 8:37 AM"; a time on its own reads "8:37 AM". Always a 12-hour clock with AM/PM, and never seconds.

Both panels get it from one place: `AppServiceProvider::configureDateTimeDisplay()` sets `defaultDateDisplayFormat`, `defaultDateTimeDisplayFormat` and `defaultTimeDisplayFormat` on `Table` and `Schema` via `configureUsing()`. So a Filament `->date()`, `->dateTime()` or `->time()` call takes no format argument — Filament's own defaults are 24-hour and carry seconds. Never pass a format at a call site to get the house format; if the house format changes, it changes in that one method.

The guest app formats its own times in the browser: `resources/js/lib/time.ts` and the `useClockTime()` hook turn the `HH:MM` the server sends into a 12-hour reading in the guest's language. A time of day crosses the wire as the stored `HH:MM`, never as a built string.

Pinned by `tests/Feature/PanelDateTimeFormatTest.php` (both the Table and the Schema path) and by `resources/js/tests/guest-menu.test.tsx`.
