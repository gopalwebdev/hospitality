...
- **`OrderStatus`:** `Placed`, `Accepted`, `Ready`, `Served`, `Cancelled` — one line from taken to handed over, with cancelled off to the side rather than at its end. **`next()` is the whole flow**, and it is the only place that knows what follows what: `AdvanceOrder` moves an order to whatever it returns and every button reads its wording from `advanceLabel()`, so a step added between two others needs nothing else changed. Three questions the rest of the application asks, and they are deliberately not one:
  - **`isOpenToChanges()`** — Placed alone. Nothing has been made to those lines yet, so ReviseOrder and the Change button hang on it (`Order::canBeChanged()`, which adds "and nothing has been paid").
  - **`isUnderway()`** — Placed, Accepted or Ready. *Is there still work to do?* This is what the **floor** draws and what cancelling is allowed on. A served order leaves the floor even though it has not been paid for; there is nothing left to call off, and giving one back is a refund.
  - **`isLive()`** — anything but Cancelled. *Does this still count?* This is what **money** asks: a served order owes exactly what it owed. Everything that once asked `isPlaced()` and meant this asks it now.

  `label()` reads Placed as **"Pending"**, because that is the word a floor uses for an order nobody has picked up. See `.ai/rules/order-taking.md`.
...
- **`StockMovementReason`:** `Restock`, `Count`, `OrderPlaced`, `OrderCancelled`, `OrderRevised`, with `label()` and `color()` for the history table. `heldByOrderValues()` is `OrderPlaced` **and** `OrderRevised` together, and it exists because an order changed before it was accepted has both: it hands everything back (`OrderRevised`) and takes the new basket (`OrderPlaced`), so what it is still holding is the **net** of those rows. CancelOrder and ReviseOrder both read it that way; summing the takes alone would put stock back twice.
...
- **`LocationActivity`:** `Ready`, `Pending`, `Preparing`, `Clear` — what is happening at a location right now, **worked out from its orders and never stored**. The `PaymentState` shape, and `.ai/rules/locations.md` forbids a `locations.status` to back it; named for activity rather than status for exactly that reason. **The order the cases are declared in is the order of urgency**, and `mostUrgent()` reads it off `cases()` — Ready first, because the food is made and somebody is waiting for it to be carried over; then Pending, which nobody has picked up; then Preparing, already in hand. Changing what a floor shouts about is changing that order and nothing else.
