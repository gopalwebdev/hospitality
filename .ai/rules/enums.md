...
- **`OrderStatus`:** `Placed`, `Accepted`, `Cancelled`, and the line that matters runs between the first two. A **placed** order is one nobody has picked up yet, so staff may still change what is on it; **accepting** it is the kitchen saying it is being made, and from there its lines are fixed. `isOpenToChanges()` is that one question, asked by `Order::canBeChanged()`, by ReviseOrder under its lock and by the panel's Change button. `isLive()` is the other: everything that used to ask `isPlaced()` and meant "this order still counts" asks it instead, because an accepted order still owes money, still shows at its room, still takes a payment and can still be cancelled — `liveValues()` is the whereIn. The later steps a kitchen moves an order through (preparing, ready, served) are still not here. See `.ai/rules/order-taking.md`.
