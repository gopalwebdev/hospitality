...
So `tax_rate_basis_points` is a plain nullable integer on `menu_items` and `menu_combos` — not on add-on options, which are taxed at their item's rate — and a non-nullable one on `tenant_settings`. A tenant types the percentage its accountant gives it.
