...
This is about **Spatie roles and permissions specifically**, because of that cache — it is not a ban on `->relationship()`. An ordinary relationship has no cache behind it: `MenuItemForm`'s add-ons repeater uses `->relationship()` on purpose so an item and its add-ons are written in one save, and `ChargeForm`'s menus checklist does too — with its options overridden to this tenant's menus and a rule refusing any other id, because `charge_menu` has no tenant to check.
