...
- **Tax:** the GSTIN, then a **This is a union territory (UTGST, not SGST)** toggle (`is_union_territory`), because a tenant states where it is and nothing infers it from an address (`.ai/rules/enums.md`); it is `live()`, and the second rate's label follows it, reading SGST or UTGST. It was a three-way select over a `GstTreatment` enum until IGST was dropped — a bill here is always an intra-state supply, so what was left was a yes-or-no. Then **CGST** and **SGST** as separate percentages, because that is how GST is levied and how an invoice has to show it; the two are this tenant's default, and an item with a `tax_rate` of its own is halved at that instead. The two are `live(onBlur: true)` and a `Text` prime under them adds them up as they are typed, so nobody types the total as well. A **Use this one rate for everything, ignoring item codes** toggle (`tax_overrides_item_rates`, renamed from "Tax every item at this rate") makes the tenant's rate beat an item's own, and the two rate fields are **editable only while it is on** — with it off, every item takes its rate from the code picked on it, so a number typed there would not be what anything is taxed at. They stay `dehydrated()` while disabled, because the stored rate is still what a **charge** is taxed at; `prices_include_tax` says whether the menu's prices already carry it. `Settings::readableRates()` / `::storableRates()` are the only place a rate is converted, so the rounding is done once.
- **Opening hours:** seven rows, one per `App\Enums\Weekday`, each a day name, a **Closed** toggle and two `ClockTimePicker`s. The toggle is `live()` and the two times are hidden — and stop being required — on a day the tenant is closed, replaced by a quiet "Closed all day" so the row does not read as one that failed to render. Only the first row carries labels; the rest `hiddenLabel()` them, so it reads as a table rather than as 28 labelled fields. Each row is a **twelve**-column `Grid` — name 2, toggle 2, each time 4 — and it **must** call `->gridContainer()`: these are container-query breakpoints, and without it `@md` never matches, every day falls back to the two-column `default`, and the week renders as fourteen half-empty rows. That is exactly what happened, and the outer Contact/Tax grid on the same page had it right the whole time.
