...
**Locations** are one generic module — `App\Enums\LocationKind` is `Room`, `Table` or `Area` — rather than separate Rooms and Tables, because this is one product for hotels, restaurants and hospitals. A hotel lists rooms, a restaurant tables, and both list a few delivery points like a pool or an entrance. It is a **flat** list: a `Zone` kind and a self-referencing `parent_id` were built to group rooms by floor and removed again on the project owner's instruction. A tenant's `type` decides which kinds it is offered and nothing else. `orders.location_id` names one and `orders.location_name` keeps a copy, so renaming or deleting a location never rewrites an order. Assigning a guest to a room or table is a **later** feature. See `.ai/rules/locations.md`.

**Staff take orders too, now.** The tenant panel's Orders page is read two ways — as a **list** and as the **floor** (every location at once, with what is open at each) — and a **Take order** counter is reached from a floor card, so an order over the phone or at the desk is the same `PlaceOrder` a guest's own phone would call. `Permission::OrderCreate` is what both hang on, and `Role::Staff` holds it. See `.ai/rules/order-taking.md`.
