...
A menu carries two **rails** as well as its categories: the items the tenant leads with (`menu_items.is_featured`, ordered by `featured_position`) and its combos. Where they sit among the categories is the tenant's own decision — `menus.featured_position` and `menus.combos_position` put them in the same number space as `menu_categories.position`, and `Menu::readingOrder()` merges the three. A menu nobody has arranged still opens with its featured items and then its combos, which is what those two columns default to. A featured item still appears under its own category further down, so a guest scrolling finds it where they expect it.
