...
The tabs (`ListMenuItems::getTabs()`) are **All**, **Items** and **Service requests**, then **Out of stock**, **Veg**, **Egg** and **Non-veg** — the quick cuts the project owner asked for beside the kind. A tab combines with whatever filters are set, and every badge comes from one query of conditional counts, deferred until the page has rendered. There is **no service request column**: the tabs say it, and the project owner had the column removed. True-or-false columns read **Yes** / **No** in words under a heading phrased as the question ("Featured?"), not an icon — the project owner found the icons unreadable.
...
Every ordered list in the panel uses Filament's own `->reorderable('position')` drag and drop, with its trigger relabelled by `App\Filament\Tables\Reordering::trigger()`: "Rearrange" enters drag mode, "Done" leaves it. That is a menu's outline (its blocks, categories and sub-categories), the tables its rows open (a category's items, the featured items, the combos), home screen rows, the tiles in a row, and charges. The menus list is deliberately not among them: menus are not ordered by hand, and it reads by name (`.ai/rules/menus.md`). Nor is the add-on groups list: it is a library read by name, and a group's place is dragged on each item that offers it (`.ai/rules/add-on-groups.md`).
