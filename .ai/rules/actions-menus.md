...
- An **addition** cannot move at all. It is edited in a repeater inside the dish that owns it, and `MenuItemAdditionObserver` refuses one whose dish belongs to another tenant.
...
The invariant that a dish leaving a menu stops being featured lives in `MenuItemObserver` rather than in whatever writes the dish, because it has to hold however the dish is written — including from a form that has just been told `is_featured` is true. `MoveCategoryToMenu` still unfeatures its branch itself, because moving a category changes no dish's `menu_category_id` and the observer cannot see it.
