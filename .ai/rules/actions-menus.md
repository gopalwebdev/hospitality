...
- It comes back **`invalid`** when an option is not an available option of one of the item's groups, when more of one is asked for than its group allows (`MenuAddOnGroup::quantityAllowedFor()`: the option's `max_quantity` while the group allows quantities, and one otherwise), when a group's picks, each counted by quantity, fall outside its minimum and maximum, or when the basket holds fewer or more of an item or combo than one order may (`min_quantity`, `max_quantity`). That last count is across every line the item or combo is on, so it flags every one of those lines.
