...
Standing instruction from the project owner: every table has exactly **one** migration, `create_<table>_table`, and a change to a table is an edit to that file — never a new `add_`, `alter_` or `rename_` migration. The history was squashed to this shape, `0001_01_01_000000_create_tenants_table.php` through `0001_01_01_000033_create_stock_movements_table.php`, numbered in foreign key order. When `menu_item_additions` was deleted the tables after it moved down one, so the three add-on tables end at `000028`; orders and stock follow them (`000029` orders, `000030` order_lines, `000031` order_line_choices, `000032` order_charges, `000033` stock_movements). A new table takes the next number (rename the timestamped file `make:migration` writes). Laravel's multi-table stubs (users and sessions, cache and cache_locks, jobs, job_batches and failed_jobs) and Spatie's five permission tables are split one table per file as well.
...
`tenants_type_is_known`, `charges_calculation_is_known`, `menu_blocks_type_is_known`, `orders_status_is_known`, `order_lines_type_is_known` and `stock_movements_reason_is_known` are built from `TenantType::cases()`, `ChargeCalculation::cases()`, `MenuBlockType::cases()`, `OrderStatus::cases()`, `OrderLineType::cases()` and `StockMovementReason::cases()`, so adding a case is an enum case and a `migrate:fresh` — though a new calculation also needs its line in `charges_value_matches_calculation`. A constraint may read an enum here only because each table's one migration is edited in place rather than replayed against an older enum.
