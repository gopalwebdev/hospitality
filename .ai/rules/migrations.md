...
Postgres is the only engine, so a rule a CHECK constraint can express is written as one as well as in the model and the form — a raw `ALTER TABLE ... ADD CONSTRAINT ... CHECK (...)` after `Schema::create()`, in the table's own migration. What exists: money, positions and role limits are never negative (Postgres has no unsigned integers, so `unsignedInteger` alone promises nothing); rates are 0–10000 basis points; a combo holds an item at least once; a menu's service window is both times or neither; a category is not its own parent; a tile's action and its destination match; an item's diet marks are filled exactly when it is not a service request (`menu_items_diets_match_service_request`) and never contradict each other (`menu_items_diets_are_consistent`, whose conflicting pairs the migration builds from `Diet::goesWith()`); a charge's number is in the one column its calculation names (`charges_value_matches_calculation`); an add-on group's maximum picks is blank or 1–99 (`menu_add_on_groups_max_picks_in_range`), an item's own cap on a group it links is blank or 1–99 (`menu_item_add_on_groups_max_picks_in_range`), an option's cap per item is 1–99 (`menu_add_on_options_max_per_item_in_range`), and an item's and a combo's maximum per order is blank or 1–99 (`menu_items_max_per_order_in_range`, `menu_combos_max_per_order_in_range`); a count is blank or never negative (`menu_items_stock_not_negative`, `menu_add_on_options_stock_not_negative`) and an item with none left is never available (`menu_items_none_left_is_not_available`); a stock movement names exactly one row and changes it by something (`stock_movements_names_one_thing`, `stock_movements_change_not_zero`); a tenant's two GST halves are each 0–10000 basis points and never add up to more than 10000 (`tenant_settings_tax_rates_in_range`); a day a tenant is closed keeps no hours and a day it opens keeps both (`tenant_opening_hours_closed_has_no_times`); an order's money is never negative and it has `cancelled_at` exactly when it is cancelled (`orders_cancelled_at_matches_status`); an order line fills only the key its type names (`order_lines_key_matches_type`); a tenant's type, a charge's calculation and a menu block's type are ones their enums know; and GST adds up — an order's `cgst + sgst + igst` equals its `tax` (`orders_tax_parts_add_up`), a line's and a charge's three rates add to its `tax_rate` (`order_lines_tax_rates_add_up`, `order_charges_tax_rates_add_up`), and a tenant's and an order's `gst_treatment` is one `GstTreatment` knows.
...
`tenants_type_is_known`, `charges_calculation_is_known`, `menu_rails_type_is_known`, `orders_status_is_known`, `order_lines_type_is_known`, `stock_movements_reason_is_known` and `tenant_opening_hours_weekday_is_known` are built from `TenantType::cases()`, `ChargeCalculation::cases()`, `MenuRailType::cases()`, `OrderStatus::cases()`, `OrderLineType::cases()`, `StockMovementReason::cases()` and `Weekday::cases()`, so adding a case is an enum case and a `migrate:fresh` — though a new calculation also needs its line in `charges_value_matches_calculation`. A constraint may read an enum here only because each table's one migration is edited in place rather than replayed against an older enum.
