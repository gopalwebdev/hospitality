...
`tenants_type_is_known`, `charges_calculation_is_known`, `menu_blocks_type_is_known`, `orders_status_is_known`, `order_lines_type_is_known`, `stock_movements_reason_is_known` and `tenant_opening_hours_weekday_is_known` are built from `TenantType::cases()`, `ChargeCalculation::cases()`, `MenuBlockType::cases()`, `OrderStatus::cases()`, `OrderLineType::cases()`, `StockMovementReason::cases()` and `Weekday::cases()`, so adding a case is an enum case and a `migrate:fresh` — though a new calculation also needs its line in `charges_value_matches_calculation`. A constraint may read an enum here only because each table's one migration is edited in place rather than replayed against an older enum.
