...
Identity includes the tenant's **type**, a hotel or a restaurant (`App\Enums\TenantType`). It has no default, so onboarding has to choose, and it stays editable afterwards on the project owner's decision — nothing a tenant sets up depends on it yet. The table shows it as a badge and filters on it.
