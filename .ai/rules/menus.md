...
| --- | --- | --- |
| a category or sub-category | `CategoryItemsRelationManager`: its items — create and edit in a full-width slide-over (add-on groups, tax and stock included), adjust stock, stock history, delete | `position` |
...
Every modal an item is created or edited in is `Width::Full` — on the Items page, in a category's table and in the featured items table — because the project owner found the stacked form wasted the screen. `MenuItemForm` is a grid container (`'@4xl' => 3`): what the item is across two thirds (name and diet, category and service request, description), its price, its tax and its stock stacked in compact sections in the last third, and the add-on groups it offers in a full-width row underneath. The container breakpoint stacks the same form into one column wherever it is given less room.

The add-on groups row is a table repeater on `addOnGroupLinks`: a searchable select per row, listing the tenant's groups labelled with their rule ("Choose your bread — Required · Choose 1"), `distinct()`, dragged into the order a guest reads them, and able to make a new group without leaving the item — beside a second field, "Maximum on this item", which caps that group's picks on this item alone, blank following the group's own maximum. See `.ai/rules/add-on-groups.md`.
