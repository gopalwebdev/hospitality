...
`MenuResource::getRecordSubNavigation()` puts four pages across the top of one menu record (`SubNavigationPosition::Top`), in this order: **Arrangement** (`ArrangeMenu`), **Menu** (`EditMenu`: name, description, hours, showing), **Featured items** (`ManageMenuFeaturedItems`) and **Combos** (`ManageMenuCombos`).
...
- The items, which are what an admin actually looks for, were on a different page entirely, so nothing on the menu's own page showed what was *in* a category.
...
`MenuArrangementTable` lists, in the order a guest reads them: the **featured rail**, the **combos rail**, every **category**, every **sub-category** under its category, and every **item** under the heading it is filed on. Indentation is the tree — `nameHtml()` pads by depth — and one drag moves anything. An item row's type reads "Service request" or "Item".
...
Items are listed and dragged here but **not edited** here: the item form carries a repeater bound to a relationship, which needs a real record behind the schema. Every item row links to the items page instead (`openItems`), which is where an item has always been edited.
...
## Featured items are a flag on the item, ordered on their own tab
`menu_items.is_featured` plus `featured_position` are what a menu leads with; `ManageMenuFeaturedItems` is where they are dragged into order. It hangs off `Menu::menuItems()`, a HasManyThrough that reaches items through their categories because an item carries its category and its tenant, never its menu.

Nothing is created, deleted **or featured** there. The page has no actions at all: it lists the featured items and lets them be dragged, which is the one thing the item's own form cannot do. Whether an item is featured is the `is_featured` toggle on that form — see `.ai/rules/actions-menus.md`. Where the rail *sits* is a third question again, answered by dragging its row on the arrangement.

`featured_position` is deliberately separate from `position`, which orders an item inside its category. An item answers both at once and the two orders are unrelated.
