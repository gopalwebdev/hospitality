---
paths:
  - 'app/Actions/Tenants/**'
---

# Tenants

## Roles are held per account, not per tenant
Spatie teams are off, so a user's roles are global to their account. Someone staffing two tenants has one set of roles, and changing them from one tenant panel would change what they can do at the other. SetTenantUserRoles is the only path a tenant panel takes to roles: it refuses when the user staffs more than one tenant, and filters to Role::assignableWithinTenant() so a role carrying a product team permission (tenant.manage, role.manage, permission.manage) can never be assigned from a tenant panel, whatever the form submits. Scoping roles per tenant means enabling spatie teams; do not work around this rule instead.

## InheritParentTenant is the tenant boundary between a row and its parent
The schema has no composite keys (no indexes — `.ai/rules/migrations.md`), so `InheritParentTenant` is what keeps a child row on its parent's tenant: the `saving` observer of every menu and home-screen child row calls it, and it fills a blank `tenant_id` from the parent or throws a LogicException for a parent that belongs to another tenant. It must stay query-free for a saved row whose parent key and tenant are both unchanged — rearranging a menu saves every row with only a few columns fetched, and strict mode throws on reading one that was not. A new tenant-owned child table needs an observer that calls it. See `.ai/rules/models.md`.
