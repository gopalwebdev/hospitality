...
A price column carries its struck-through "was" price as the column's `description()` rather than taking a column of its own, which would be empty for every row that is not on offer — most of them. The same trick carries an item's description under its name. A price of zero prints "Complimentary", not "₹0.00".
