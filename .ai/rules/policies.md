...
`OrderPolicy` has a `cancel()` that no generator writes, asked by the cancel action's `->authorize('cancel')`, and answers `update`, `delete` and `deleteAny` with false outright: what was ordered is a record rather than a draft, so an order that should not stand is cancelled. Its `create()` answered false too while ordering was the guest app's alone; it is `order.create` now that staff take orders from the panel's own counter (`.ai/rules/order-taking.md`), and it is what both that page and the Orders list's New order button hang on.
