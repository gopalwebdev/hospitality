...
`OrderPolicy` has three methods no generator writes — `cancel()`, `advance()` and `change()`, each `order.manage` and each asked by its action's `->authorize(...)`; creating is `order.create` and is the separate thing of bringing a new order into existence, while those three are working one that already exists. Whether *this* order may still be changed is `Order::canBeChanged()` and ReviseOrder's to enforce; the policy only says who may try. Its `cancel()`, and answers `update`, `delete` and `deleteAny` with false outright: what was ordered is a record rather than a draft, so an order that should not stand is cancelled. Its `create()` answered false too while ordering was the guest app's alone; it is `order.create` now that staff take orders from the panel's own counter (`.ai/rules/order-taking.md`), and it is what both that page and the Orders list's New order button hang on.
