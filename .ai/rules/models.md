...
Standing instruction from the project owner: a model holds relationships, scopes, casts and small accessors, with short docblocks — no essays. It never registers events with `booted()` or `static::saving(...)` closures. Behaviour that has to run when a row is written goes in an observer under `app/Observers/`, created only when a model actually needs one, and attached with `#[ObservedBy([...])]`. Today that is User, MenuCategory, MenuItem, MenuItemAddition, MenuCombo, MenuComboItem, HomeTile and Charge.
...
The menu is generic: an extra pillow and a bottle of water sit on the same card. `menu_items.is_service` is a plain boolean — an `ItemKind` enum was built first and replaced by the boolean on the project owner's instruction — and `menu_items.diet` is `App\Enums\Diet` (veg / egg / non-veg), **null exactly when the item is a service request**.

The pairing is held three times, like a tile's action and destination: `MenuItemObserver` clears the diet of a service request (which is what lets an item become one) and throws when anything else has none; the `menu_items_diet_matches_service` CHECK constraint says `is_service = (diet IS NULL)`; and `MenuItemForm` hides and stops requiring the diet while the "Service request" toggle is on. The observer skips a saved item whose flag and diet are both untouched, because `ApplyMenuArrangement` renumbers items it loaded with only a few columns and strict mode throws on a column never fetched. The model mirrors `is_service => false` in `$attributes` because the observer reads it before the insert.
