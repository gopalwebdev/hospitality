...
- A bill's charges are `Charge::query()->where('tenant_id', ...)->active()->forMenu($menuId)->inMenuOrder()`. Name the tenant even so: `forMenu()` reads the pivot alone. `Charge::amountOn($subtotalMinorUnits)` is what `App\Actions\Menus\QuoteBasket` adds to a basket: a share rounded once, or the fixed sum.
