...
The pairing is held three times, like a tile's action and destination: `MenuItemObserver` clears the marks of a service request (which is what lets an item become one), throws when anything else has none, and throws on two that contradict each other; the `menu_items_diets_match_service_request` CHECK constraint says `is_service_request = (diets IS NULL)` and `menu_items_diets_are_consistent` says the rest; and `MenuItemForm` hides and stops requiring the marks while the "Service request" toggle is on. The observer skips a saved item whose flag and marks are both untouched, so a save that loaded only some of an item's columns does not throw under strict mode for a column it never fetched. The model mirrors `is_service_request => false` in `$attributes` because the observer reads it before the insert.
...
- A bill's charges are `Charge::query()->where('tenant_id', ...)->active()->forMenu($menuId)->inMenuOrder()`. Name the tenant even so: `forMenu()` reads the pivot alone. `Charge::amountOn($subtotal)` is what `App\Actions\Menus\QuoteBasket` adds to a basket: a share rounded once, or the fixed sum.
