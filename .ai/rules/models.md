...
- A bill's charges are `Charge::query()->where('tenant_id', ...)->active()->forMenu($menuId)->inMenuOrder()`. Name the tenant even so: `forMenu()` reads the pivot alone. `Charge::amountOn($subtotal)` is what `App\Actions\Menus\PriceBasket` adds to a basket: a share rounded once, or the fixed sum.
