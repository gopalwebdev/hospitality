...
Keep the related columns in step when writing rows. The factories exist for exactly this — `MenuCategoryFactory::inMenu()` / `::under()`, `MenuItemFactory::inCategory()` / `::service()` / `::limitedPerOrder()`, `MenuAddOnGroupFactory::ofTenant()` / `::choosing()` / `::allowingQuantities()`, `MenuAddOnOptionFactory::inGroup()` / `::asDefault()`, `MenuItemAddOnGroupFactory::linking()`, `MenuComboFactory::onMenu()` / `::limitedPerOrder()`, `MenuComboItemFactory::pairing()`, `MenuBlockFactory::onMenu()` / `::ofType()`, `HomeTileFactory::inRow()` / `::openingMenu()`, `ChargeFactory::onMenus()` — and setting the halves independently is refused by the observers.
