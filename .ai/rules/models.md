...
- A bill's charges are `Charge::query()->where('tenant_id', ...)->active()->forMenu($menuId)->inMenuOrder()`. Always name the tenant: `forMenu()` matches every tenant's all-menus charges on its own. `Charge::amountOn($subtotalMinorUnits)` is what `App\Actions\Menus\QuoteBasket` adds to a basket: a share rounded once, or the fixed sum.
