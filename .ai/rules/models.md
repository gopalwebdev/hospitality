...
Keep the related columns in step when writing rows. The factories exist for exactly this — `MenuCategoryFactory::inMenu()` / `::under()`, `MenuItemFactory::inCategory()` / `::service()`, `MenuAddOnGroupFactory::ofTenant()` / `::choosing()` / `::allowingQuantities()`, `MenuAddOnOptionFactory::inGroup()` / `::asDefault()`, `MenuItemAddOnGroupFactory::linking()`, `MenuComboFactory::onMenu()`, `MenuComboItemFactory::pairing()`, `MenuBlockFactory::onMenu()` / `::ofType()`, `HomeTileFactory::inRow()` / `::openingMenu()`, `ChargeFactory::onMenus()` — and setting the halves independently is refused by the observers.
