...
Standing instruction from the project owner: a model holds relationships, scopes, casts and small accessors, with short docblocks — no essays. It never registers events with `booted()` or `static::saving(...)` closures. Behaviour that has to run when a row is written goes in an observer under `app/Observers/`, created only when a model actually needs one, and attached with `#[ObservedBy([...])]`. Today that is User, MenuCategory, MenuItem, MenuAddOnOption, MenuItemAddOnGroup, MenuCombo, MenuComboItem, MenuRail, HomeTile, Charge, Location and OrderPayment.
...
The menu is `menus` → `menu_categories` (both levels of section) → `menu_items`, plus `menu_combos` and `menu_combo_items` and `menu_rails` hanging off a menu and the add-on library beside it — `menu_add_on_groups` → `menu_add_on_options`, offered on items through `menu_item_add_on_groups` — and every one of them carries `tenant_id` directly as well as reaching it through its parent. The home screen is two — `home_rows` → `home_tiles` — and does the same, as does a tile for the menu it opens. `charges` carry `tenant_id` too.

The schema has no indexes (`.ai/rules/migrations.md`), so there are no composite foreign keys to hold the two halves together. `App\Actions\Tenants\InheritParentTenant` does instead, from the `saving` observer of every child row: a row with no tenant takes its parent's, and a row whose parent belongs to another tenant throws a LogicException — a category on another tenant's menu, an item in another tenant's category, an option in another tenant's group, an item offering another tenant's group, a combo line naming another tenant's item, a block on another tenant's menu, a tile in another tenant's row or opening another tenant's menu. It returns early for a saved row whose parent key and tenant are both unchanged, so renumbering a list costs no query and reads no column it was not given. Pinned in `MenuManagementTest`, `MenuCategoryTreeTest`, `MenuComboTest`, `AddOnGroupManagementTest` and `HomeRowManagementTest`.

`locations` and `order_payments` derive theirs the same way — a location from its parent zone, an allocation from its payment — and `order_payments.tenant_id` is deliberately **not fillable**, so anything writing an allocation lets the observer set it.

It runs on `saving` because Laravel fires that before `creating`, the event in which Filament's tenancy stamps `tenant_id` — and Filament does that only for a *resource* model (Menu, MenuItem, MenuAddOnGroup, HomeRow, Charge), overwriting whatever was set. It never stamps the rows a relation manager, a repeater or a hand-built action writes (categories, combos, blocks, combo lines, options, item links, tiles), which is why those derive theirs. A group made from the item form's select is stamped by `MenuAddOnGroupForm::createFromItemForm()` itself. A raw `DB::table()` insert goes around all of it.
...
## There are two self-referencing trees, and they are the same shape
`menu_categories.parent_id` (below) and `locations.parent_id` are both nullable, both self-referencing, and both capped at **two levels** by an observer — `MenuCategoryObserver` and `LocationObserver`, which refuse a row as its own parent, a parent that itself has a parent, and a parent belonging elsewhere. That is deliberate: one pattern here, not two. A third tree should look like these.

Where they differ is what may be a parent. Any top-level category may hold sub-categories; only a `LocationKind::Zone` may hold locations, asked through `canHoldChildren()` rather than compared by name. See `.ai/rules/locations.md`.
...
Keep the related columns in step when writing rows. The factories exist for exactly this — `MenuCategoryFactory::inMenu()` / `::under()`, `MenuItemFactory::inCategory()` / `::service()` / `::limitedPerOrder()` / `::stocked()`, `MenuAddOnGroupFactory::ofTenant()` / `::choosing()`, `MenuAddOnOptionFactory::inGroup()` / `::asDefault()` / `::upTo()` / `::stocked()`, `MenuItemAddOnGroupFactory::linking()` / `::capping()`, `OrderFactory::onMenu()` / `::cancelled()`, `OrderLineFactory::inOrder()`, `MenuComboFactory::onMenu()` / `::limitedPerOrder()`, `MenuComboItemFactory::pairing()`, `MenuRailFactory::onMenu()` / `::ofType()`, `HomeTileFactory::inRow()` / `::openingMenu()`, `ChargeFactory::onMenus()`, `TenantOpeningHourFactory::ofTenant()` / `::on()` / `::between()` / `::closed()` — and setting the halves independently is refused by the observers.

Never resolve an item's currency or tax rate through `$item->tenant->settings`: that is a lazy load, which `Model::shouldBeStrict()` throws on in local and in the test suite and which is an N+1 down a list of items. `App\Models\Concerns\HasPricing` holds `currency()`, `taxRate()`, `formattedPrice()` and `formattedComparePrice()` once for both `MenuItem` and `MenuCombo`; every reader takes an optional override, and a list should pass one, because every row shares the tenant's answer.

## An item is something to order or a service request, and only one of them carries a diet
The menu is generic: an extra pillow and a bottle of water sit on the same card. `menu_items.is_service_request` is a plain boolean — an `ItemKind` enum was built first and replaced by the boolean on the project owner's instruction, and the column was later renamed from `is_service`, which read as though it had something to do with a service charge — and `menu_items.diets` is a `jsonb` list of `App\Enums\Diet` (veg / vegan / egg / non-veg) cast with `AsEnumCollection`, **null exactly when the item is a service request** and holding at least one mark otherwise. A list because most vegetarian items are vegan too; which marks may combine is `Diet::goesWith()` and which one a guest reads is `MenuItem::dietMark()` (`.ai/rules/enums.md`).

The pairing is held in two places now, like a tile's action and destination: `MenuItemObserver` clears the marks of a service request (which is what lets an item become one), throws when anything else has none, and throws on two that contradict each other; it was backed by `menu_items_diets_match_service_request` and `menu_items_diets_are_consistent` until every CHECK constraint in the schema was dropped (`.ai/rules/migrations.md`), so the observer is now the only guard against a write that goes around the model; and `MenuItemForm` hides and stops requiring the marks while the "Service request" toggle is on. The observer skips a saved item whose flag and marks are both untouched, so a save that loaded only some of an item's columns does not throw under strict mode for a column it never fetched. The model mirrors `is_service_request => false` in `$attributes` because the observer reads it before the insert.
...
- `calculation` is `App\Enums\ChargeCalculation`: `Percentage` keeps its number in `rate`, `FixedAmount` in `amount`, and `ChargeCalculation::valueColumn()` is the single place that says which. `ChargeObserver` clears the column a calculation does not use and throws when the one it uses is blank; `charges_value_matches_calculation` says the same in the database.
- A charge is on the menus attached to it in `charge_menu`, and no others. An `applies_to_all_menus` flag existed and was removed on the project owner's instruction. A menu created later is on no charge until it is picked on one.
...
- A bill's charges are `Charge::query()->where('tenant_id', ...)->active()->forMenu($menuId)->inMenuOrder()`. Name the tenant even so: `forMenu()` reads the pivot alone. `Charge::amountOn($subtotal)` is what `App\Actions\Baskets\PriceBasket` adds to a basket: a share rounded once, or the fixed sum.
...
`menu_add_on_options` deliberately keeps a plain boolean `is_available`. An option that has run out is simply not offered, so there is nothing for the extra cases to say there — though a required group left with too little to pick takes its item off the guest menu (`.ai/rules/add-on-groups.md`). It has no tax rate either: an add-on is taxed at its item's rate.

Items and options can also be **counted** (`stock_quantity`, null is not counted). None left marks an item `OutOfStock` and hides an option through `MenuAddOnOption::scopeAvailable()` without touching `is_available`. A saved row's count is changed only through `ApplyStockChanges` — see `.ai/rules/inventory.md`.

## Guest-facing text is a translated JSON column, unique on English
Menu, MenuCategory, MenuItem, MenuAddOnGroup, MenuAddOnOption, HomeRow, HomeTile and Charge store their guest-facing text with spatie/laravel-translatable: the column is `jsonb` holding one key per App\Enums\Locale case, and the model declares `public array $translatable`. Use App\Models\Concerns\HasTranslatedNames, never Spatie's trait directly — it adds the one thing the package leaves open, which is that English (Locale::default()) is privileged.
...
Money stays an integer all the way out of PHP. `MenuItem::formattedPrice()` exists for the Filament tables, which are server rendered; the guest app is sent `price` and format it themselves — see `.ai/rules/js.md`. Do not add a `formattedX()` accessor for an Inertia payload.

## The tenant is a Tenant, and every key to it is tenant_id
The tenant boundary is `App\Models\Tenant` on `tenants`, whatever `tenants.type` holds — this is one common product, and nothing outside `App\Enums\TenantType` names a kind of business. Settings are `tenant_settings`, the week it opens is `tenant_opening_hours` (a row per `App\Enums\Weekday` — see `.ai/rules/app.md`), the roster pivot is `tenant_user`, and the product team's permission is `tenant.manage`.

`TenantOpeningHour` and `Menu` both read wall-clock times out of a `time` column, so the pair of readers is `App\Models\Concerns\ReadsClockTimes`: `normalisedTime()` pads a value to `HH:MM:SS` for comparing, `clockReading()` cuts it back to `HH:MM` for reading. A time column hands back seconds nobody wants to see, and a picker sets none.
