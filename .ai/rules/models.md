...
Keep the related columns in step when writing rows. The factories exist for exactly this — `MenuCategoryFactory::inMenu()` / `::under()`, `MenuItemFactory::inCategory()` / `::service()`, `MenuAddOnGroupFactory::ofTenant()` / `::choosing()`, `MenuAddOnOptionFactory::inGroup()`, `MenuItemAddOnGroupFactory::linking()`, `MenuComboFactory::onMenu()`, `MenuComboItemFactory::pairing()`, `MenuBlockFactory::onMenu()` / `::ofType()`, `HomeTileFactory::inRow()` / `::openingMenu()`, `ChargeFactory::onMenus()` — and setting the halves independently is refused by the observers.
...
`menu_add_on_options` deliberately keeps a plain boolean `is_available`. An option that has run out is simply not offered, so there is nothing for the extra cases to say there — though a required group left with too little to pick takes its item off the guest menu (`.ai/rules/add-on-groups.md`).
