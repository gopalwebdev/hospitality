...
`order_payments` derives its tenant the same way, from its payment, and `order_payments.tenant_id` is deliberately **not fillable**, so anything writing an allocation lets the observer set it.
